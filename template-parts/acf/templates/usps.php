...
<section classw-full>
	...
	<div class="grid md:grid-cols-2 xl:grid-cols-3 gap-8">
		...
		if ( have_rows( 'add_list' ) ) :
			...
			echo '<ul class="md:col-span-2 xl:col-span-1 grid gap-4 mb-8 md:mb-0">';
			...
			while ( have_rows( 'add_list' ) ) :
				...
				printf(
					'<li class="w-fit py-2 grid place-content-center sm:text-xl font-medium border border-text-color rounded-full px-5 mx-auto xl:mx-0">%s
					</li>',
					esc_html( $list )
				);
		...
		if ( $rating_title || $rating_text ) {
			echo '<div class="xl:col-span-1 bg-white p-7 flex flex-col justify-between gap-4 text-text-color rounded-20">';
			if ( $rating_title ) {
		...
		if ( $review_text || $review_number || $review_link ) {
			if ( $review_link ) {
				printf(
					'<a href="%s" target="_blank" class="xl:col-span-1 w-full p-7 flex flex-col justify-between bg-blue-light text-white rounded-20">
				...
				printf(
					'<div class="xl:col-span-1 w-full p-7 flex flex-col justify-between bg-blue-light text-white rounded-20">
						<p class="text-white">%s</p>
						<div class="flex gap-2 items-center text-3xl sm:text-6xl font-bold">%s%s</div>
					</div>',
					wp_kses_post( $review_text ),
